...
if (!empty($_REQUEST['scan']) || !empty($_REQUEST['newscan']) || !empty($_REQUEST['rescanerrors'])) {
	...
	while ($row = mysql_fetch_array($result)) {
		...
		$ThisLine  = '<tr>';
		$ThisLine .= '<td><a href="'.htmlentities('demo.browse.php?filename='.rawurlencode($row['filename']), ENT_QUOTES).'">views</a></td>';
	...
	echo '<table border="1" cellspacing="0" cellpadding="3">';
	echo '<tr><th>views</th><th>Why</th><td><b>Actual filename</b><br>(click to play/edit file)</td><td><b>Correct filename (based on tags)</b>'.(empty($_REQUEST['autofix']) ? '<br>(click to rename file to this)' : '').'</td></tr>';
	...
	while ($row = mysql_fetch_array($result)) {
		...
		if ($ActualFilename != $PatternFilename) {
			...
			echo '<tr>';
			echo '<td><a href="'.htmlentities('demo.browse.php?filename='.rawurlencode($row['filename'])).'">views</a></td>';
